...
public final class GenericRecordWrapperJsonProvider extends AbstractMessageReaderWriterProvider<GenericRecordWrapper> {

    private final GenericAvroCodec avroCodec = new GenericAvroCodec();

    @Override
    public boolean isWriteable(Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType) {
        return GenericRecordWrapper.class.isAssignableFrom(type) && mediaType.getSubtype().endsWith("json");
    }
    ...
    public void writeTo(GenericRecordWrapper wrapper, Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType, MultivaluedMap<String, Object> httpHeaders, OutputStream entityStream) {
        avroCodec.encodeJson(wrapper.getRecord(), entityStream);
    }
